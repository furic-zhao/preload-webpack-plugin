...
class PreloadPlugin {
  ...
  addLinks(compilation, htmlPluginData) {
    ...
    // to prefetch async chunks for a specific entry point
    if (options.include.entries && !options.include.entries.includes(htmlPluginData.plugin.options.filename.split('.')[0])) {
      return htmlPluginData;
    }
    ...
    const htmlChunks = options.include === 'allAssets'
      ...
      : extractedChunks.filter((chunk) => doesChunkBelongToHTML({
        chunk,
        compilation,
        htmlAssetsChunks: Object.values(htmlPluginData.plugin.options.chunks)
      }))
